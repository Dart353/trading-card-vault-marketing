...
### ❌ Hardcoding `id` on Form Inputs

`Form.Control` passes a generated `id` via `props` that matches the `Form.Label`'s `for` attribute. Overriding it with a hardcoded `id` breaks the label-input association, making labels non-clickable.

```typescript
// ✗ WRONG - Hardcoded id overrides props, label click won't focus input
<Form.Field {form} name="email">
  <Form.Control>
    {#snippet children({ props })}
      <Form.Label>Email</Form.Label>
      <Input {...props} id="email" bind:value={$formData.email} />
    {/snippet}
  </Form.Control>
</Form.Field>

// ✓ CORRECT - Let props provide the id
<Form.Field {form} name="email">
  <Form.Control>
    {#snippet children({ props })}
      <Form.Label>Email</Form.Label>
...
- [ ] No hardcoded `id` on inputs — let `{...props}` from `Form.Control` handle it
