...
├── Service.ts                  # Base class (REQUIRED - all services extend this)
├── user/
│   ├── UserService.ts
│   └── UserServiceTypes.ts
└── product/
...
All services MUST extend the `Service` base class from `Service.ts`:
...
// 1. construct it
// 2. add it to the services object
// 3. export it
...
- **Service instances** — ALWAYS import from `$lib/services` (the index), never from individual service files
- **Service types** — Import from the specific types file: `$lib/services/user/UserServiceTypes`

```typescript
// ✓ CORRECT
import { userService } from '$lib/services';
import type { UserServiceTypes } from '$lib/services/user/UserServiceTypes';

// ✗ WRONG - Never import instances from individual service files
...
import type { UserServiceTypes } from './UserServiceTypes';
...
- **Extends Service base class** (REQUIRED)
...
- [ ] Services use `export default class` and `extends Service`
- [ ] Services use Restura-generated functions
- [ ] Service instances imported from `$lib/services`, not individual files
