...
**`$derived` values are mutable in Svelte 5.** You can reassign or mutate a `$derived` variable — it acts as a default/seed that re-syncs from its expression when dependencies change, but local mutations are allowed between updates. Do NOT flag `$derived` mutation as a bug.
...
**Only service files** (`src/lib/services/**`) may import from `generated/apiRequests`. Components (`.svelte`), pages (`+page.svelte`), layouts (`+layout.svelte`, `+layout.ts`), and hooks (`hooks.server.ts`) must NEVER import `ApiRequestV1` directly — they call service methods instead.
...
// ✗ WRONG - NEVER import ApiRequestV1 outside of services
import { ApiRequestV1 } from '../generated/apiRequests';
let user = await ApiRequestV1.getUserMe();
...
- **Callback props**: `onSubmit`, `onClick` (prop names use `on` prefix)
- **Parent callbacks**: `handleSubmit`, `handleClick` (implementations use `handle` prefix)
...
❌ Importing `ApiRequestV1` or `generated/apiRequests` outside of service files
...
❌ Using `on` prefix for parent callback implementations (`onSubmit` instead of `handleSubmit`)
❌ Using `handle` prefix for component prop names (`handleClick` instead of `onClick`)  
